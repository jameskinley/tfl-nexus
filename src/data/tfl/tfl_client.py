...
class TflClient:
    ...
    def get_disruptions_by_mode(self, modes: List[str]):
        endpoint = f"Line/Mode/{','.join(modes)}/Disruption"
        return self._execute_request(endpoint)
    ...
    def get_arrivals(self, line_ids: List[str], stop_point_id: str, direction: str = "all"):
        endpoint = f"Line/{','.join(line_ids)}/Arrivals/{stop_point_id}"
        params = {}
        if direction != "all":
