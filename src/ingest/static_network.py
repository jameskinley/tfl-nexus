"""
Static Network Ingestion (Phase 1)

Ingests the static network topology from TfL APIs:
- Stops (with PostGIS geometry)
- Services (tube/DLR/tram lines)
- Edges (directional connections between stops)

This module replaces src/data/ingest_pipeline.py with cleaner functional structure.
"""
...
from .schema import Stop, Service, Edge
from src.data.tfl.tfl_client import TflClient


def ingest_stops(session: Session, tfl_client: TflClient, modes: List[str]) -> Dict[str, int]:
    """
    ...
    try:
        ...
        for mode in tqdm(modes, desc="Processing modes", unit="mode"):
            try:
                ...
                response = tfl_client.get_stops_by_mode([mode])
...
def ingest_services(session: Session, tfl_client: TflClient, modes: List[str]) -> Dict[str, int]:
    """
    ...
    try:
        ...
        lines = list(tfl_client.get_lines_by_mode(modes))
...
def ingest_edges(session: Session, tfl_client: TflClient, 
                 stop_mapping: Dict[str, int], service_mapping: Dict[str, int]) -> int:
    """
    ...
    Args:
        session: SQLAlchemy session
        tfl_client: Configured TfL API client
        stop_mapping: naptan_id -> stop_id mapping
        service_mapping: tfl_line_id -> service_id mapping
    ...
    try:
        ...
        for line_id, service_id in tqdm(service_mapping.items(), desc="Processing route sequences", unit="line"):
            try:
                # Fetch route sequence
                route_sequence = tfl_client.get_route_sequence(line_id, direction="all")
                ...
                for sequence in stop_sequences:
                    ...
                    for i in range(len(stop_points) - 1):
                        try:
                            ...
                            from_stop_id = stop_mapping.get(from_stop_naptan)
                            to_stop_id = stop_mapping.get(to_stop_naptan)
...
def verify_network(session: Session):
    """
